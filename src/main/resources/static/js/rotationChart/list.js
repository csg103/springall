...
function loadData(index, size){
    layui.use(['table','layer', 'laytpl'], function(){
        ...
        table.render({
            elem: '#vipTable'
            ,height: 450
            ,url: ctx + 'rotationChart/list?index=' + index + '&size=' + size //数据接口
            ,cols: [[
                {field: 'desc', title: '轮播图描述'}
        ...
        table.on('tool(vip_table)', function(obj){
            ...
            if (layEvent == 'detail'){
                ...
                x_admin_show("轮播图修改",ctx + "rotationChart/edit?id=" + data.id, 500, 600)
            } else if (layEvent == 'del'){
                layer.confirm('删除轮播图' + data.name + '?', function(index){
                    $.ajax({
                        url: ctx + 'rotationChart/delete',
                        type:'post',
                        dataType:'json',
                        data:{
                            id:data.id
