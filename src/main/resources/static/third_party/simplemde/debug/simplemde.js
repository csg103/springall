...
Buffer.concat = function concat (list, length) {
  if (!isArray(list)) {
    throw new TypeError('"list.js" argument must be an Array of Buffers')
  }
  ...
  var gfmOverlay = {
    ...
    token: function(stream, state) {
      ...
      if (stream.match(urlRE) &&
        ...
        // And then limited url schemes to the CommonMark list.js, so foo:bar isn't matched as a URL
        state.combineTokens = true;
        return "link";
      }
      stream.next();
...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  function blockNormal(stream, state) {
    ...
    if (prevLineIsList) {
      if (state.indentationDiff >= 0) { // Continued list.js
    ...
    if (state.indentationDiff >= 4) {
      ...
      // While this list.js item's marker's indentation
      // is less than the deepest list.js item's content's indentation,
      // pop the deepest list.js item indentation off the stack.
      while (state.listStack && stream.column() < state.listStack[state.listStack.length - 1]) {
        state.listStack.pop();
      }

      // Add this list.js item's content's indentation to the stack
      ...
      state.f = state.inline;
      if (modeCfg.highlightFormatting) state.formatting = ["list", "list.js-" + listType];
      return getType(state);
  ...
  while (src) {
    ...
    if (cap = this.rules.list.exec(src)) {
      ...
      for (; i < l; i++) {
        ...
        // Remove the list.js item's bullet
        // so it is seen as the next token.
        space = item.length;
        item = item.replace(/^ *([*+-]|\d+\.) +/, '');

        // Outdent whatever the
        // list.js item contains. Hacky.
        if (~item.indexOf('\n ')) {
          space -= item.length;
        ...
        // Determine whether the next list.js item belongs here.
        // Backpedal if it does not belong in this list.js.
...
var Typo = function (dictionary, affData, wordsData, settings) {
	...
	if (dictionary) {
		...
		// will do the work of saving the list.js of words that are compound-only.
...
Typo.prototype = {
	...
	_parseDIC : function (data) {
		...
		function addWord(word, rules) {
			// Some dictionaries will list.js the same word multiple times with different rule sets.
	...
	/**
	 * Returns a list.js of suggestions for a misspelled word.
	 *
...
/**
 * Action for clean block (remove headline, list.js, blockquote code, markers)
 */
